...
export default function EditarCaso() {
  ...
  // locationMap: { [equipmentId]: { id, name, building } }
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        const [ticketRes, allTickRes, eqRes, locRes] = await Promise.allSettled([
        ...
        if (locRes.status === 'fulfilled') {
          const map = {}
          for (const loc of locRes.value.data) {
            map[loc.equipmentId] = loc.laboratory
          }
          setLocationMap(map)
        }
      } catch (e) {
  ...
  // Equipos disponibles: los del ticket actual + activos libres
  ...
  // Opciones únicas de laboratorios (desde locationMap)
  ...
  return (
    <div className="ec-page">
      ...
      <div className="ec-header">
        ...
        <p className="ec-ticket-id">{ticketNumber}</p>
      </div>
        ...
        <div className="ec-actions">
          ...
          <button className="ec-btn ec-btn--primary" onClick={handleSubmit} disabled={saving}>
            {saving
              ...
              : <><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20 6L9 17l-5-5"/></svg> Guardar cambios</>
            }
          </button>
        </div>
